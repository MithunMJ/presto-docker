...
config=$PRESTO_HOME/etc/catalog/adls-wasb-site.xml
...
cat > $PRESTO_HOME/etc/catalog/cosmosdb.properties <<EOF
...
cat > $PRESTO_HOME/etc/catalog/azuresql.properties <<EOF
...
cat > $PRESTO_HOME/etc/catalog/postgresql.properties <<EOF
